...
var xml2object = module.exports = function(elements, source, saxOptions, strict) {
	elements = elements || [];
	...
	this.source = source;
	this.saxStream = sax.createStream(strict, saxOptions);
	...
	var closeTag = function (name) {
		...
		if (inObject && inObjectName === name &&
			isNoDuplicateNameInAncestors(ancestors, name)) {
	...
	function isNoDuplicateNameInAncestors(ancestors = [], name) {
		let count = 0;
		ancestors.forEach(a => {
			if (Object.keys(a).includes(name)) ++count;
		});
		return count < 2;
